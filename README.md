...
- 본 서버는 네이버 부동산의 **비공식 내부 API**를 사용합니다. 네이버에서 공식적으로 제공·보장하는 인터페이스가 아니며, 네이버의 인가를 받지 않았습니다.
- 비공식 API 사용은 네이버 이용약관과 충돌할 가능성이 있습니다. 응답 구조·정책·차단 정책이 사전 통보 없이 변경될 수 있습니다.
- **네이버가 차단·중단·시정을 요청할 경우 사용자는 즉시 사용을 중단해야 합니다.**
- **상업적 이용을 엄격히 금지**하며, 개인적인 학습·조회 용도에 한해 사용해주세요.
- API 호출은 **요청 간 최소 0.5초 딜레이**를 두고 수행합니다 (`config.REQUEST_DELAY_SEC`). 과도한 호출은 IP 차단의 원인이 되므로 절대 줄이지 마세요.
- 사용에 따른 모든 법적·기술적 책임은 사용자 본인에게 있습니다. 본 저장소의 작성자·기여자는 어떠한 책임도 지지 않습니다.
